...
Here are the [homework exercises from the book](/files/homework_docs/McElreath-Ch-9-11-homework.pdf)
...
For exercise 9M3, the question is ambiguous, about whether you should 
uze the original `dexp(1)` or the `dunif(0,1)` from 9M1 as the prior for
...
To check yourself, for 11E1 the log-odds is `r round(log(0.35 / (1 - 0.35)), 3)`, for 11E2 the probability is `r round(logistic(3.2), 3)`, and for 11E3 the proportional change in the odds is `r round(exp(1.7), 3)`

#### Exercise 11M7

For 11M7, I recommend using the following code to compare the 
`precis`es of the two models (`m11.4` is the model on page 330, and 
`m11.4a` is the `quap` version model that you should write for this
exercise).
